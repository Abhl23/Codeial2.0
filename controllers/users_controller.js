...
module.exports.profile = async function (req, res) {
  try {
    ...
    const areFriends = req.user.friendships.indexOf(req.params.id);

    return res.render("user_profile", {
      title: "User Profile",
      profile_user: user,
      areFriends: (areFriends == -1 ? false : true),
    });
  } catch (err) {
    req.flash("error", err);
      ...
      User.uploadedAvatar(req, res, function (err) {
        ...
        if (req.file) {
          if (
            ...
            // checks if the avatar actually exists in both user and in directory
